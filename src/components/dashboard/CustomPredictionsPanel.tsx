...
export function CustomPredictionsPanel({ groupId }: { groupId: string }) {
  ...
  const handleSubmit = async (cpId: string) => {
    ...
    setPredictions((prev) =>
      prev.map((cp) => cp.id === cpId ? { ...cp, userAnswer: option } : cp)
    );
    setTimeout(() => setSaved((p) => ({ ...p, [cpId]: false })), 2000);
  };

  // Only show active predictions: OPEN or recently resolved (last 7 days)
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        {visible.map((cp) => {
          const maxCount = cp.answerCounts
            ? Math.max(1, ...Object.values(cp.answerCounts))
            : 1;
            ...
            <div key={cp.id} className="card">
              ...
              <div className="flex items-start justify-between gap-2 mb-3">
                ...
                <span className={`badge shrink-0 ${
                  ...
                  cp.isLocked ? "bg-red-100 text-red-700" :
              ...
              {!cp.isLocked && cp.status === "OPEN" && (
                <p className="text-xs text-gray-400 mb-3">
                  <Countdown lockTime={cp.lockTime} />
                </p>
              )}

              {/* OPEN + not locked: radio selection */}
              {cp.status === "OPEN" && !cp.isLocked && (
                <>
                    ...
                    {cp.options.map((opt) => (
                      ...
                      >
                        <input
                          ...
                          name={cp.id}
                          value={opt}
                          checked={selected[cp.id] === opt}
                          onChange={() => setSelected((p) => ({ ...p, [cp.id]: opt }))}
                          className="sr-only"
                        />
                        <span className={`w-4 h-4 rounded-full border-2 flex-shrink-0 ${
                          selected[cp.id] === opt ? "border-fifa-blue bg-fifa-blue" : "border-gray-300"
                        }`} />
                        {opt}
                      </label>
                    ))}
                  ...
                  <button
                    ...
                    disabled={saving[cp.id] || !selected[cp.id]}
              ...
              {/* RESOLVED: show correct answer + user result */}
              {cp.status === "RESOLVED" && (
              ...
              {/* LOCKED + not resolved: show distribution of picks */}
              {cp.status === "OPEN" && cp.isLocked && cp.answerCounts && (
                <div className="space-y-2">
                  ...
                  {cp.options.map((opt) => {
                    ...
                    return (
                      <div key={opt} className={`rounded-lg border px-3 py-2 text-sm ${isUserAnswer ? "border-fifa-blue bg-blue-50" : "border-gray-200"}`}>
                        <div className="flex items-center justify-between mb-1">
                          <span className={`font-medium ${isUserAnswer ? "text-fifa-blue" : "text-gray-700"}`}>
                            {opt}{isUserAnswer ? " ← your pick" : ""}
                          </span>
